...
for entry in smashdata["struct"]["list"]["struct"]:
    ...
    while True:
        ...
        attrs = [
            f"chara_3_{0}_scale",
            f"chara_3_{0}_offset_x",
            f"chara_3_{0}_offset_y"
        ]
        ...
        eyecount = int(data["eye_0_flash_count"])
        ...
        for e in range(eyecount):
            eyex.append(
                int(data[f"eye_0_flash{e}_pos_x"]))
            eyey.append(
                int(data[f"eye_0_flash{e}_pos_y"]))

        try:
            t = magic.from_file(
                f'/home/joao/StreamHelperAssets/games/ssbu/vs_renders/chara_3_{codename}_0{global_id}.png')
...
full_config = json.load(
    open("/home/joao/StreamHelperAssets/games/ssbu/vs_renders/config.json"))

full_config["eyesights"] = out2["eyesights"]
full_config["rescaling_factor"] = out2["rescaling_factor"]

with open("/home/joao/StreamHelperAssets/games/ssbu/vs_renders/config.json", 'w') as outfile:
