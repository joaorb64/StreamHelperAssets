...
# file_list = glob("../../games/ggst/full/*.png", recursive=True)
with open("../../games/ggst/full/config.json", "rt", encoding="utf-8") as config_file:
    full_config = json.loads(config_file.read())
with open("../../games/ggst/base_files/config.json", "rt", encoding="utf-8") as config_file:
...
for character_name in main_config["character_to_codename"].keys():
    ...
    image_filename = f"../../games/ggst/full/{full_config['prefix']}{codename}{full_config['postfix']}0.png"
    if os.path.isfile(image_filename):
        image = Image.open(image_filename, "r").convert("RGBA")
        height = image.height
        rescaling_factor[codename] = {"0": 1600.0/height}
...
with open("../../games/ggst/full/config.json", "wt", encoding="utf-8") as config_file:
