...
# file_list = glob("../../games/mkwld/base_files/icon/*.png", recursive=True)
with open("../../games/mkwld/base_files/icon/config.json", "rt", encoding="utf-8") as config_file:
    full_config = json.loads(config_file.read())
with open("../../games/mkwld/base_files/config.json", "rt", encoding="utf-8") as config_file:
...
for character_name in main_config["character_to_codename"].keys():
    ...
    for i in range(0,20):
        image_filename = f"../../games/mkwld/base_files/icon/{full_config['prefix']}{codename}{full_config['postfix']}{i:02}.png"
        if os.path.isfile(image_filename):
            image = Image.open(image_filename, "r").convert("RGBA")
            height = image.height
            rescaling_factor[codename][str(i)] =  100.0/height
...
with open("../../games/mkwld/base_files/icon/config.json", "wt", encoding="utf-8") as config_file:
