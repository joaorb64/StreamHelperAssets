...
# file_list = glob("../../games/pkmn/full/*.png", recursive=True)
with open("../../games/pkmn/full/config.json", "rt", encoding="utf-8") as config_file:
...
for character_name in main_config["character_to_codename"].keys():
    ...
    for i in range(20):
        image_filename = f"../../games/pkmn/full/{full_config['prefix']}{codename}{full_config['postfix']}{i}.png"
        if os.path.isfile(image_filename):
            image = Image.open(image_filename, "r").convert("RGBA")
            height = image.height

            current_costume_rescaling_factor = 512.0/height
...
with open("../../games/pkmn/full/config.json", "wt", encoding="utf-8") as config_file:
