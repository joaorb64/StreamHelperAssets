...
# file_list = glob("../../games/sg/full/*.png", recursive=True)
with open("../../games/sg/full/config.json", "rt", encoding="utf-8") as config_file:
    full_config = json.loads(config_file.read())
with open("../../games/sg/base_files/config.json", "rt", encoding="utf-8") as config_file:
...
for character_name in main_config["character_to_codename"].keys():
    ...
    image_filename = f"../../games/sg/full/{full_config['prefix']}{codename}{full_config['postfix']}0.png"
    image = Image.open(image_filename, "r").convert("RGBA")
    height = image.height
    rescaling_factor[codename] = {"0": 2009.0/height}
    print(codename, rescaling_factor[codename]["0"])
full_config["rescaling_factor"] = rescaling_factor
with open("../../games/sg/full/config.json", "wt", encoding="utf-8") as config_file:
