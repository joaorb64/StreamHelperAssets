...
dustloop_page = requests.get(
    "https://www.dustloop.com/wiki/index.php?title=Granblue_Fantasy_Versus", timeout=30)
...
def get_icon_from_character_name(character_name):
    ...
    for tag in list_img_tags:
        try:
            ...
            alt_text = None
        if (character_name in alt_text) and ("Icon.png".lower() in alt_text.lower()) and ("Icon.png" in tag["src"]):
...
def get_portrait(character_name, page_link):
    ...
    for tag in character_page_links_tag_list:
        link = tag["href"]
        if ("_Portrait" in link or "_Potrait" in link) and ("File:GBVS" in link):
    ...
    for tag in portrait_page_links_tag_list:
        text = tag.get_text()
        link = tag["href"]
        if (("Original file" in text) and ("_Portrait" in link)) or (("GBVS" in text) and ("_Portrait.png" in text) and ("_Portrait" in link)):
            break
        if (("Original file" in text) and ("_Potrait" in link)) or (("GBVS" in text) and ("_Potrait.png" in text) and ("_Potrait" in link)):
            break
