...
dustloop_page = requests.get(
    "http://www.dustloop.com/wiki/index.php?title=Guilty_Gear_-Strive-")
...
def get_icon_from_character_name(character_name):
    ...
    for tag in list_img_tags:
        try:
            ...
            alt_text = None
        if (character_name in alt_text) and ("GGST" not in alt_text):
...
def get_portrait(character_name, page_link):
    ...
    for tag in character_page_links_tag_list:
        link = tag["href"]
        if ("_Portrait" in link) and ("File:GGST" in link):
