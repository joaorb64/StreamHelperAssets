...
dustloop_page = requests.get(
    "http://www.dustloop.com/wiki/index.php?title=Persona_4:_Arena_Ultimax_2.5", timeout=30)
...
character_table = dustloop_soup.findAll(
    'div', {'class': f'center', 'style': "width:auto; margin-left:auto; margin-right:auto;"})
        ...
        for character_name_part in character_name_split:
            if (character_name_part in alt_text.replace("PSho", "Minazuki")) and ("Icon.png" in tag['src']) and ("px" not in tag["srcset"].split(", ")[-1]):
                ...
                if ("Labrys" in character_name):
                    if ("Shadow" in character_name) and ("SLabrys" in alt_text):
                        srcset = tag["srcset"].split(", ")
                    if ("Shadow" not in character_name) and ("SLabrys" not in alt_text):
                        srcset = tag["srcset"].split(", ")
...
def get_portrait(character_name, page_link):
    ...
    for tag in character_page_links_tag_list:
        link = tag["href"]
        if ("_Portrait" in link) and ("File:P4U" in link):
    ...
    for tag in portrait_page_links_tag_list:
        text = tag.get_text()
        link = tag["href"]
        if (("Original file" in text) and ("_Portrait" in link)) or (("P4U" in text) and ("_Portrait.png" in text) and ("_Portrait" in link)):
            break
...
def get_all_links():
    ...
    for tag in list_characters_tag:
        character_name = tag["title"].split("/")[-1].encode("ascii", "ignore").decode()
