...
info_url = "https://wiki.gbl.gg/w/Art_of_Fighting_3"
base_url = f"{info_url}/Rody_Birts"
...
def get_icon_from_character_name(character_name):
    ...
    for tag in list_img_tags:
        try:
            ...
            alt_text = None
        if (character_name in alt_text) and ("Select" in tag["src"]):
...
def get_portrait(character_name, page_link):
    ...
    for tag in character_page_links_tag_list:
        link = tag["href"]
        if ("Win_Screen" in link) and ("File:AOF3" in link):
    ...
    for tag in portrait_page_links_tag_list:
        text = tag.get_text()
        link = tag["href"]
        if ("Win_Screen.png" in text) and "Win_Screen" in link:
            break
...
def get_all_links():
    ...
    for character_tag in character_icons_table:
        ...
        for href_tag in href_tags:
            if "Art of Fighting 3" not in href_tag["title"]:
