...
info_url = "https://wiki.gbl.gg/w/Dan-Ku-Ga"
base_url = f"{info_url}/Kazuya"
...
def get_portrait(character_name, page_link):
    ...
    for tag in character_page_links_tag_list:
        link = tag["href"]
        if (link.lower().endswith("dkg_"+character_name.lower().replace(' ', "_").replace('j._mccoy', 'mccoy')+".png")) and ("File:DKG" in link):
    ...
    for tag in portrait_page_links_tag_list:
        text = tag.get_text()
        link = tag["href"]
        if (text.lower().endswith("dkg_"+character_name.lower().replace(' ', "_").replace('j._mccoy', 'mccoy')+".png") or "Original file" in text) and link.lower().endswith("dkg_"+character_name.lower().replace(' ', "_").replace('j._mccoy', 'mccoy')+".png"):
            break
...
def get_all_links():
    ...
    for character_tag in character_icons_table:
        ...
        for href_tag in href_tags:
            if "Dan-Ku-Ga" not in href_tag["title"]:
