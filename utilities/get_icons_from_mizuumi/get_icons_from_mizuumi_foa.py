...
info_url = "https://wiki.gbl.gg/w/Fight_of_Animals"
base_url = f"{info_url}/Crowrilla"
...
def get_icon_from_character_name(character_name):
    ...
    for tag in list_img_tags:
        try:
            ...
            alt_text = None
        if (character_name in alt_text) and ("Profile" in tag["src"]):
...
def get_portrait(character_name, page_link):
    ...
    for tag in character_page_links_tag_list:
        link = tag["href"]
        if ("Full" in link) and ("File:FOA" in link):
    ...
    for tag in portrait_page_links_tag_list:
        text = tag.get_text()
        link = tag["href"]
        if ("Full.png" in text or "Original file" in text) and "Full" in link:
            break
...
def get_all_links():
    ...
    for character_tag in character_icons_table:
        ...
        for href_tag in href_tags:
            if "Fight of Animals" not in href_tag["title"]:
