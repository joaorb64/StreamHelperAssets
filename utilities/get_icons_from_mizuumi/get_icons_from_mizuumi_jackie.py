...
base_url = "https://wiki.gbl.gg/w/Jackie_Chan_in_Fists_of_Fire/Lau"
...
def get_portrait(character_name, page_link):
    ...
    for tag in character_page_links_tag_list:
        link = tag["href"]
        if ("Portrait" in link) and ("File:JCFoF" in link):
    ...
    for tag in portrait_page_links_tag_list:
        text = tag.get_text()
        link = tag["href"]
        if ("Original file" in text) and "Portrait" in link:
            break
...
def get_all_links():
    ...
    for character_tag in character_icons_table:
        ...
        for href_tag in href_tags:
            if "Jackie Chan in Fists of Fire" not in href_tag["title"]:
...
def generate_configs(character_dict):
    ...
    credits = 'Assets ripped from Mizuumi Wiki (https://wiki.gbl.gg/w/Jackie_Chan_in_Fists_of_Fire)'
    version = "1.0"
