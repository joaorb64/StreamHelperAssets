...
info_url = "https://wiki.gbl.gg/w/Kirby_Fighters_2"
base_url = f"{info_url}/Sword"
...
def get_portrait(character_name, page_link):
    ...
    for tag in character_page_links_tag_list:
        link = tag["href"]
        if (link.lower().endswith("kf2_"+character_name.lower().replace(' ', "_").replace('-', '')+".png")) and ("File:KF2" in link):
    ...
    for tag in portrait_page_links_tag_list:
        text = tag.get_text()
        link = tag["href"]
        if (text.lower().endswith("kf2_"+character_name.lower().replace(' ', "_").replace('-', '')+".png") or "Original file" in text) and link.lower().endswith("kf2_"+character_name.lower().replace(' ', "_").replace('-', '')+".png"):
            break
...
def get_all_links():
    ...
    for character_tag in character_icons_table:
        ...
        for href_tag in href_tags:
            if "Kirby Fighters 2" not in href_tag["title"]:
