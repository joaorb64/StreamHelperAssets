...
info_url = "https://wiki.gbl.gg/w/Melty_Blood/MBAACC"
base_url = f"{info_url}/Aoko_Aozaki"
...
for tag in parse_tables:
    for tag2 in tag.findAll(
            'span', {'style': f'float:left; margin-left:1px; margin-right:1px;'}):
        if "charselect" in str(tag2):
            character_icons_table.append(tag2)
...
def get_icon_from_character_name(character_name):
    ...
    for tag in list_img_tags:
        try:
            ...
            alt_text = None
        if (character_name in alt_text) and ("charselect" in tag["src"]):
...
def get_portrait(character_name, page_link):
    ...
    for tag in portrait_page_links_tag_list:
        text = tag.get_text()
        link = tag["href"]
        if ("banner" in text) and "banner" in link and text.endswith(".png"):
            break
...
def get_all_links():
    ...
    for character_tag in character_icons_table:
        ...
        for href_tag in href_tags:
            if "Melty Blood/MBAACC" not in href_tag["title"]:
                if "Crescent" not in href_tag["title"] and "Full" not in href_tag["title"] and "Half" not in href_tag["title"]:
