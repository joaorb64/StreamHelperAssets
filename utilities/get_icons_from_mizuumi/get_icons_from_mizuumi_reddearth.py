...
info_url = "https://wiki.gbl.gg/w/Red_Earth/War-Zard"
base_url = f"{info_url}/Leo"
...
def get_icon_from_character_name(character_name):
    ...
    for tag in list_img_tags:
        try:
            ...
            alt_text = None
        if (character_name in alt_text) and ("RE" in tag["src"]):
...
def get_portrait(character_name, page_link):
    ...
    for tag in character_page_links_tag_list:
        link = tag["href"]
        if ("Portrait" in link) and ("File:RE" in link):
    ...
    for tag in portrait_page_links_tag_list:
        text = tag.get_text()
        link = tag["href"]
        if ("Portrait.gif" in text) and "Portrait" in link:
            break
...
def get_all_links():
    ...
    for character_tag in character_icons_table:
        ...
        for href_tag in href_tags:
            if "Red Earth/War-Zard" not in href_tag["title"]:
