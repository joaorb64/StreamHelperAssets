...
info_url = "https://wiki.gbl.gg/w/The_Rumble_Fish_2"
base_url = f"{info_url}/Aran"
...
def get_portrait(character_name, page_link):
    ...
    for tag in character_page_links_tag_list:
        link = tag["href"]
        if ("MAIN" in link) and ("File:TRF2" in link):
    ...
    for tag in portrait_page_links_tag_list:
        text = tag.get_text()
        link = tag["href"]
        if (("MAIN" in text and ".png" in text) or "Original file" in text) and "MAIN" in link:
            break
...
def get_all_links():
    ...
    for character_tag in character_icons_table:
        ...
        for href_tag in href_tags:
            if "The Rumble Fish 2" not in href_tag["title"]:
