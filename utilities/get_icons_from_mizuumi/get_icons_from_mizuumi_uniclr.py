...
base_url = "https://wiki.gbl.gg/w/Under_Night_In-Birth/UNICLR/Hyde"
...
def get_portrait(character_name, page_link):
    ...
    for tag in portrait_page_links_tag_list:
        text = tag.get_text()
        link = tag["href"]
        if ("Original file" in text) and "Profile-" in link:
            break
...
def get_all_links():
    ...
    for character_tag in character_icons_table:
        ...
        for href_tag in href_tags:
            if "Under Night In-Birth" not in href_tag["title"]:
...
def generate_configs(character_dict):
    ...
    credits = 'Assets ripped from Mizuumi Wiki (https://wiki.gbl.gg/w/Under_Night_In-Birth/UNICLR)'
    version = "1.0"
