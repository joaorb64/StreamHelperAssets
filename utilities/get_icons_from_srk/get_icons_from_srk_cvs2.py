...
base_url = "https://srk.shib.live/w/Capcom_vs_SNK_2"
...
for tag in parse_tables:
    if ("Akuma" in str(tag)) or ("Terry" in str(tag)):
...
def get_icon_from_character_name(character_name):
    ...
    for tag in list_img_tags:
        try:
            ...
            alt_text = None
        if (character_name in alt_text) and ("square" in tag["src"]):
...
def get_all_links():
    ...
    for character_tag in character_icons_table:
        ...
        for href_tag in href_tags:
            if "Capcom vs SNK 2/" in href_tag["title"]:
                list_characters_page.append(f'https://srk.shib.live{href_tag["href"]}')
                list_characters.append(re.sub("[\(\[].*?[\)\]]", '', href_tag.text).lstrip().rstrip())
...
def generate_configs(character_dict):
    ...
    credits = 'Assets ripped from SRK Wiki (https://srk.shib.live/w/Capcom_vs_SNK_2)'
    version = "1.0"
...
def download_all_images(character_dict: dict, icon_config_dict, portrait_config_dict):
    for character_name in character_dict.keys():
        ...
        icon_filename = f'{icon_folder_name}/{icon_config_dict.get("prefix")}{character_data.get("codename")}{icon_config_dict.get("postfix")}0.jpg'
