...
base_url = "https://wiki.supercombo.gg/w/Project_Justice"
...
def get_icon_from_character_name(character_name):
    ...
    for tag in list_img_tags:
        try:
            ...
            alt_text = None
        if (character_name in alt_text) and ("Face" in tag["src"]):
...
def get_portrait(character_name, page_link):
    ...
    for tag in character_page_links_tag_list:
        link = tag["href"]
        if (".gif" in link.lower()) and ("file:project_justice" in link.lower()):
    ...
    if portrait_wiki_link:
        ...
        for tag in portrait_page_links_tag_list:
            text = tag.get_text()
            link = tag["href"]
            if (("Original file" in text) or (".gif" in text)) and "project_justice" in link.lower():
                break
...
def get_all_links():
    ...
    for character_tag in character_icons_table:
        ...
        for href_tag in href_tags:
            if "Project Justice/" in href_tag["title"]:
...
def generate_configs(character_dict):
    ...
    credits = f'Assets ripped from Supercombo Wiki ({base_url})'
    version = "1.0"
...
def download_all_images(character_dict: dict, icon_config_dict, portrait_config_dict):
    for character_name in character_dict.keys():
        ...
        portrait_filename = f'{portraits_folder_name}/{portrait_config_dict.get("prefix")}{character_data.get("codename")}{portrait_config_dict.get("postfix")}0.gif'
