...
base_url = "https://wiki.supercombo.gg/w/Tatsunoko_vs_Capcom:_Ultimate_All_Stars"
...
for tag in parse_tables:
    if ("Batsu" in str(tag)) or ("Doronjo" in str(tag)) or ("PTX-40A" in str(tag)):
...
def get_portrait(character_name, page_link):
    ...
    for tag in character_page_links_tag_list:
        link = tag["href"]
        if ("_art" in link.lower()) and ("file:tvc" in link.lower()):
    ...
    if portrait_wiki_link:
        ...
        for tag in portrait_page_links_tag_list:
            text = tag.get_text()
            link = tag["href"]
            if (("Original file" in text) or (".png" in text)) and "_art" in link.lower():
                break
...
def get_all_links():
    ...
    for character_tag in character_icons_table:
        ...
        for href_tag in href_tags:
            if "Tatsunoko vs Capcom: Ultimate All Stars/" in href_tag["title"]:
...
def generate_configs(character_dict):
    ...
    credits = 'Assets ripped from Supercombo Wiki (https://srk.shib.live/w/Marvel_vs_Capcom_2)'
    version = "1.0"
